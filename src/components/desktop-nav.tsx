  ...
  return (
    <nav className="hidden items-center rounded-2xl bg-slate-100/80 p-1 md:flex" aria-label="التنقل الرئيسي">
      {items.map((item) => {
        ...
        return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`min-h-10 rounded-xl px-3 py-2.5 text-sm font-semibold transition lg:px-4 ${active ? "bg-white text-palm-700 shadow-sm" : "text-slate-600 hover:text-ink"}`}>{item.label}</Link>;
      })}
    </nav>
  );
}
