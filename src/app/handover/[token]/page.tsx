        ...
        {delivered ? (
          ...
          <>
            ...
            <section className="card mb-5">
              <div className="flex items-center justify-between border-b border-black/5 pb-4"><div><p className="text-xs text-black/40">رقم العُهدة</p><p className="mt-1 font-black">{shipment.refCode}</p></div><span className="pill">{shipment.fromCity} ← {shipment.toCity}</span></div>
              <div className="mt-4 grid grid-cols-2 gap-4 text-sm"><div><p className="text-xs text-black/40">المستلم</p><p className="mt-1 font-black">{shipment.recipientName}</p></div><div><p className="text-xs text-black/40">المسافر</p><p className="mt-1 font-black">{shipment.acceptedOffer?.traveler.name ?? "—"}</p></div><div><p className="text-xs text-black/40">آخر موعد</p><p className="mt-1 font-black">{formatDate(shipment.requestedDeliveryAt)}</p></div><div><p className="text-xs text-black/40">الجوال</p><p className="mt-1 font-black" dir="ltr">•••• {shipment.recipientPhone.slice(-4)}</p></div></div>
            </section>
