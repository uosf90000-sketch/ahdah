      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1.15fr_.85fr]">
        <div className="space-y-6">
          <section className="card">
            ...
            <div className="mt-5 rounded-2xl bg-sand-50 p-5"><p className="mb-2 text-xs font-bold text-black/40">وصف المحتويات</p><p className="whitespace-pre-wrap text-sm leading-8">{shipment.contents}</p></div>
          </section>
            ...
            <section className="card">
              ...
              <div className="space-y-4">
                {shipment.offers.filter((offer) => offer.status === "PENDING").map((offer) => {
                  ...
                  return (
                    <article className="rounded-2xl border border-black/5 p-5" key={offer.id}>
                      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><strong className="text-lg">{offer.traveler.name}</strong><BadgeCheck className="text-palm-600" size={18} /></div><p className="mt-1 text-xs text-black/45">{offer.trip.airline} · {formatDate(offer.trip.departureAt)} · {avg}</p></div><strong className="text-2xl text-palm-700">{formatMoney(offer.priceSar)}</strong></div>
                      {offer.note && <p className="mt-4 rounded-xl bg-black/[.025] p-3 text-sm leading-7 text-black/60">{offer.note}</p>}
                ...
                {!shipment.offers.some((offer) => offer.status === "PENDING") && <div className="rounded-2xl bg-black/[.025] p-7 text-center"><Plane className="mx-auto mb-3 text-palm-600" /><p className="muted">لم يصل عرض بعد. يظهر طلبك للمسافرين المطابقين تلقائيًا.</p></div>}
              </div>
            </section>
          )}
          ...
          {isAcceptedTraveler && shipment.status === "TRAVELER_ACCEPTED" && (
            <section className="card border-2 !border-palm-100">
              ...
              <form action={inspectShipmentAction} className="space-y-4" encType="multipart/form-data">
                <input type="hidden" name="shipmentId" value={shipment.id} />
                {[['matchedDescription', 'المحتويات مطابقة للوصف المكتوب'], ['matchedPhotos', 'المحتويات مطابقة لصور المرسل'], ['noProhibitedItems', 'تأكدت من عدم وجود مواد محظورة أو مخفية'], ['packageSealed', 'تم إغلاق العُهدة بعد الفحص']].map(([name, label]) => <label key={name} className="flex cursor-pointer items-start gap-3 rounded-2xl border border-black/5 p-4 font-bold hover:bg-palm-50"><input className="mt-1 h-5 w-5 accent-palm-600" type="checkbox" name={name} required /><span>{label}</span></label>)}
        ...
        <aside className="space-y-6 lg:sticky lg:top-24">
          {shipment.acceptedOffer ? (
            <section className="card">
              <div className="mb-5 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-palm-50 text-palm-700"><UserRoundCheck /></span><div><p className="text-xs font-bold text-black/40">المسافر المقبول</p><h2 className="text-lg font-black">{shipment.acceptedOffer.traveler.name}</h2></div></div>
          ...
          {shipment.acceptedOffer && (
            <section className="card text-center">
              ...
              <div className="relative mx-auto my-5 aspect-square w-52 overflow-hidden rounded-2xl border border-black/5 bg-white p-2"><Image src={`/api/qr/${shipment.qrToken}`} alt={`QR للعُهدة ${shipment.refCode}`} fill unoptimized className="object-contain p-2" /></div>
              <p className="text-xs font-bold text-black/45">المستلم: {shipment.recipientName} · ••••{shipment.recipientPhone.slice(-4)}</p>
...
function Fact({ Icon, label, value }: { Icon: typeof Box; label: string; value: string }) {
  return <div className="rounded-2xl border border-black/5 p-4"><Icon className="mb-3 text-palm-600" size={19} /><p className="text-xs font-bold text-black/40">{label}</p><p className="mt-1 text-sm font-black">{value}</p></div>;
}
...
function PhotoSection({ title, photos, verified = false }: { title: string; photos: Array<{ id: string; url: string; caption: string | null }>; verified?: boolean }) {
  return <section className="card"><div className="mb-5 flex items-center gap-2"><h2 className="text-xl font-black">{title}</h2>{verified && <span className="pill"><BadgeCheck size={14} /> موثقة</span>}</div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{photos.map((photo) => <figure className="relative aspect-square overflow-hidden rounded-2xl bg-black/5" key={photo.id}><Image src={photo.url} alt={photo.caption ?? title} fill unoptimized className="object-cover" /></figure>)}</div></section>;
}
...
function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between border-b border-black/5 pb-3 last:border-0"><span className="text-black/45">{label}</span><strong>{value}</strong></div>;
}
