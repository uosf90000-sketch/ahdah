  ...
  return (
    <div className="page-wrap section-space">
      ...
      <section className="mb-8 overflow-hidden rounded-[2rem] bg-ink p-6 text-white shadow-card sm:p-8">
        <div className="mb-7 flex items-start justify-between gap-4"><div><span className="text-xs font-bold text-blue-200">رحلتك القادمة</span><h1 className="mt-2 text-3xl font-bold">{trip.fromCity} ← {trip.toCity}</h1></div><Plane className="text-blue-200" size={34} /></div>
      ...
      <div className="mb-5 flex items-end justify-between"><div><p className="eyebrow mb-2">مطابقة ذكية</p><h2 className="text-2xl font-bold">{matches.length} عُهد مناسبة لرحلتك</h2></div><Link href="/matches" className="hidden min-h-11 items-center text-sm font-bold text-palm-700 sm:inline-flex">كل المطابقات <ArrowLeft className="inline" size={15} /></Link></div>
      {matches.length ? (
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {matches.map((shipment) => (
            <article className="card overflow-hidden !p-0" key={shipment.id}>
              ...
              <div className="p-5"><div className="mb-3 flex items-center justify-between"><strong>{CATEGORY_LABELS[shipment.category]}</strong><span className="text-xs font-bold text-slate-500">{shipment.refCode}</span></div><p className="line-clamp-2 text-sm leading-7 text-slate-600">{shipment.contents}</p><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-xs font-bold text-slate-500">قبل {formatDate(shipment.requestedDeliveryAt)}</span><Link href={`/shipments/${shipment.id}?trip=${trip.id}`} className="inline-flex min-h-11 items-center text-sm font-bold text-palm-700">عرض وتقديم سعر <ArrowLeft className="inline" size={15} /></Link></div></div>
            </article>
          ))}
        </div>
      ) : (
